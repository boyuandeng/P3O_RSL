    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.normalize_advantage_per_mini_batch:
                with torch.no_grad():
                    advantages_r_batch = (advantages_r_batch - advantages_r_batch.mean()) / (advantages_r_batch.std() + 1e-8)
                    if self.num_costs > 0:
                        mean_c = advantages_c_batch.mean(dim=0, keepdim=True)
                        std_c = advantages_c_batch.std(dim=0, keepdim=True)
                        advantages_c_batch = (advantages_c_batch - mean_c) / (std_c + 1e-8)
            ...
            if self.symmetry and self.symmetry["use_data_augmentation"]:
                ...
                # -- critic
                target_values_r_batch = target_values_r_batch.repeat(num_aug, 1)
                advantages_r_batch = advantages_r_batch.repeat(num_aug, 1)
                returns_r_batch = returns_r_batch.repeat(num_aug, 1)
                if self.num_costs > 0:
            ...
            surrogate = -torch.squeeze(advantages_r_batch) * ratio
            surrogate_clipped = -torch.squeeze(advantages_r_batch) * torch.clamp(
                ratio, 1.0 - self.clip_param, 1.0 + self.clip_param
            )
            surrogate_loss = torch.max(surrogate, surrogate_clipped).mean()
            if self.num_costs :
                ratio_e = ratio.unsqueeze(-1)
                unclipped_c = ratio_e * advantages_c_batch
                clipped_c = torch.clamp(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param).unsqueeze(-1) * advantages_c_batch
                cost_term = torch.max(unclipped_c, clipped_c)
                if self.cost_limits is not None:
                    Jc = returns_c_batch.mean(dim=0, keepdim=True)
                    cost_term = cost_term + (1.0 - self.gamma) * (Jc - self.cost_limits)
                policy_cost_loss = cost_term.clamp_min(0).mean()
            else:
                policy_cost_loss = torch.tensor(0.0, device=self.device)
            # Value function loss
            if self.use_clipped_value_loss:
                value_clipped = target_values_r_batch + (value_r_batch - target_values_r_batch).clamp(
                    -self.clip_param, self.clip_param
                )
                value_r_losses = (value_r_batch - returns_r_batch).pow(2)
                value_r_losses_clipped = (value_clipped - returns_r_batch).pow(2)
                value_r_loss = torch.max(value_r_losses, value_r_losses_clipped).mean()
            else:
                value_r_loss = (returns_r_batch - value_r_batch).pow(2).mean()
            if self.num_costs:
                if self.use_clipped_value_loss:
                    value_c_clipped = target_values_c_batch + (value_c_batch - target_values_c_batch).clamp(
                        -self.clip_param, self.clip_param
                    )
                    value_c_losses = (value_c_batch - returns_c_batch).pow(2)
                    value_c_losses_clipped = (value_c_clipped - returns_c_batch).pow(2)
                    value_c_loss = torch.max(value_c_losses, value_c_losses_clipped).mean()
                else:
